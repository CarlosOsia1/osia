...
export const SEASON_SURFACES = ['sky', 'ground', 'foliage'];
...
export function resolveSeasonTints(timeOfYear) {
    const { a, b, k } = bracket(wrap01(timeOfYear));
    const out = {};
    for (const s of SEASON_SURFACES)
        out[s] = lerpRGB(a.tints[s], b.tints[s], k);
    return out;
}
/** Estación dominante (el cuarto del año donde estamos). Para HUD/diagnóstico. */
export function seasonAt(timeOfYear) {
 ...
 */
export function applySeason(p, skyTint) {
