...
const GREEN_HUE = [110, 205];
/** Banda de tono de magentas calientes / fucsias. */
const MAGENTA_HUE = [300, 350];
 ...
 */
export function checkHouseCelestial(c) {
    const [L, a, b] = rgbToOklab(c);
    const chroma = Math.hypot(a, b);
    const h = hueDeg(a, b);
    if (L <= CLIP_BLACK_L)
        return `clipping a negro puro (L=${L.toFixed(3)})`;
    if (L >= CLIP_WHITE_L)
        return `clipping a blanco puro (L=${L.toFixed(3)})`;
...
export function lintAtmosphereParams(p) {
    const out = [];
 ...
 */
export function clampToHouseCelestial(c) {
    if (checkHouseCelestial(c) === null)
        return c;
...
//# sourceMappingURL=housePalette.js.map