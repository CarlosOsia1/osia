...
const ACCENT = hexToRGB('#cbb89a'); // --osia-champagne-500 (acento de marca)
...
const ch255 = (v) => Math.round(clamp01(v) * 255);
/** Resuelve las variables --atmo-* del HUD a partir de los params de atmósfera del frame. */
export function resolveHudAtmo(p) {
    const sky = rgbToOklab(p.skyHorizon);
    // Tono del cielo + luminancia del champán → acento que respira sin perder legibilidad.
    const tintRGB = clampToHouseCelestial(oklabToRGB(ACCENT_LAB[0], lerp(ACCENT_LAB[1], sky[1], HUE_BLEND), lerp(ACCENT_LAB[2], sky[2], HUE_BLEND)));
